...
bool loadKNNDataAndTrainKNN(void) {
    ...
    cv::FileStorage fsClassifications(QCoreApplication::applicationDirPath().toStdString()+"/../../../classifications.xml", cv::FileStorage::READ);        // open the classifications file
    ...
    cv::FileStorage fsTrainingImages(QCoreApplication::applicationDirPath().toStdString()+"/../../../images.xml", cv::FileStorage::READ);              // open the training images file
...
std::vector<PossiblePlate> detectCharsInPlates(std::vector<PossiblePlate> &vectorOfPossiblePlates) {
    ...
    for (auto &possiblePlate : vectorOfPossiblePlates) {            // for each possible plate, this is a big for loop that takes up most of the function
        ...
        // perform country pattern check
        QRegExp plateRegex("^[a-zA-Z]{3}\\d{4}$");
        QString plate = QString::fromStdString(possiblePlate.strChars);
        if(!plateRegex.exactMatch(plate))
        {
            possiblePlate.strChars = "";            // set plate string member variable to empty string
            continue;
        }
